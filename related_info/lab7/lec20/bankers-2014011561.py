...
class Bankers(object):
    ...
    def SignProcesses(self, max_, allocated_):
        ...
        self.available = self.Calcavailable()
    ...
    def ExecuteProcess(self, index):
        ...
        # check if less available than Request
        # 2014011561
        ...
        for i in range(len(self.need[index])):
            ...
            self.available[i] -= self.need[index][i]
            self.need[index][i] = 0
        return True
        # allocating END here
            ...
            for i in arr:
                if self.finished[i] is False:
                    ...
                    if self.ExecuteProcess(i):
                        ...
                        # check if at least one request can be done after previous process done. not check whole sequances.
                        # if every element of Requests can't accepted after
                        # previous process done, this mean it is not safe state
